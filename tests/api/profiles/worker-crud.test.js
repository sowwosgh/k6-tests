/**
 * Worker Profile — Full CRUD Integration Test
...
export default function (data) {
  ...
  // READ
  const readRes = http.get(`${BASE_URL}/api/worker/${id}`, { headers: ah });
  check(readRes, {
    'READ — 200': (r) => r.status === 200,
    'READ — full_name matches': (r) => { try { return r.json().full_name === 'K6 Test Worker'; } catch { return false; } },
  });
  sleep(0.3);

  // UPDATE
  const patchRes = http.patch(`${BASE_URL}/api/worker/${id}`,
    JSON.stringify({ search_status: 'open_to_offers', salary_amount: 85000, experience_years: 5 }),
    { headers: ah });
  ...
  // DELETE
  const delRes = http.del(`${BASE_URL}/api/worker/${id}`, null, { headers: ah });
  check(delRes, { 'DELETE — 200 or 204': (r) => r.status === 200 || r.status === 204 });
  sleep(0.5);

  // 404 AFTER DELETE
  check(http.get(`${BASE_URL}/api/worker/${id}`, { headers: ah }), {
    'SOFT DELETE — 404': (r) => r.status === 404,
  });

  console.log('\n✅ Worker CRUD completed');
}

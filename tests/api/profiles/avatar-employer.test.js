...
/**
 * Employer Logo Tests (avatar = logo for employer)
 * POST /api/employer/{id}/logo
 * DELETE /api/employer/{id}/logo
 */
export default function () {
  ...
  // Setup: create employer
  pause(10);
  const createRes = http.post(`${BASE_URL}/api/employer`, JSON.stringify({
    company_name: generateCompanyName('Работодатель K6 Logo'),
    inn: generateINN(),
    ...
    website: 'https://k6test-employer-logo.com',
    contact_person: 'Тестовый HR',
    contact_phone: '+7 (666) 777-88-99',
    contact_email: 'hr-logo@k6test-employer.com',
    about: 'Тест логотипа работодателя',
  }), { headers: authJsonHeaders });

  if (createRes.status !== 200 && createRes.status !== 201) {
    console.error('❌ Failed to create employer:', createRes.body); return;
  }
  ...
  // Test 2: Upload — non-existent employer
  console.log('\n❌ Test 2: Non-existent employer...');
  const uploadWrongRes = http.post(`${BASE_URL}/api/employer/999999/logo`, { logo: imageFile }, { headers: authHeaders });
  check(uploadWrongRes, { '[Upload Wrong ID] 404': (r) => r.status === 404 });

  // Test 3: Upload — unauthenticated
  console.log('\n🔒 Test 3: Unauthenticated...');
  const uploadUnauthRes = http.post(`${BASE_URL}/api/employer/${employerId}/logo`, { logo: imageFile });
  check(uploadUnauthRes, { '[Upload Unauth] 401 or 403': (r) => r.status === 401 || r.status === 403 });

  // Test 4: Delete logo — success
  console.log('\n🗑️  Test 4: Delete logo...');
  const deleteRes = http.del(`${BASE_URL}/api/employer/${employerId}/logo`, null, { headers: authHeaders });
  console.log(`Delete: ${deleteRes.status}`);
  check(deleteRes, { '[Delete] 200 or 204': (r) => r.status === 200 || r.status === 204 });

  // Test 5: Delete — non-existent employer
  console.log('\n❌ Test 5: Delete non-existent...');
  const deleteWrongRes = http.del(`${BASE_URL}/api/employer/999999/logo`, null, { headers: authHeaders });
  check(deleteWrongRes, { '[Delete Wrong ID] 404': (r) => r.status === 404 });

  // Cleanup
  http.del(`${BASE_URL}/api/employer/${employerId}`, null, { headers: authJsonHeaders });
  console.log('\n✅ Employer logo tests completed');
}

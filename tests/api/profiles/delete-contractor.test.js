...
/**
 * DELETE Contractor Profile Test
 * 
 * Tests:
 * 1. Delete own contractor profile (success)
...
export default function () {
  ...
  // ===========================================
  // Setup: Create Contractor Profile
  // ===========================================
  console.log('\n📝 Creating contractor profile...');
  const createRes = http.post(
    `${BASE_URL}/api/contractor`,
    JSON.stringify({
  ...
  // ===========================================
  // Test 1: Delete Own Contractor Profile (Success)
  // ===========================================
  console.log('\n🗑️  Test 1: Delete own contractor profile...');
  const deleteRes = http.del(
    `${BASE_URL}/api/contractor/${contractorId}`,
    null,
    { headers: authHeaders }
  );
  ...
  const deleteNonExistentRes = http.del(
    `${BASE_URL}/api/contractor/999999`,
    null,
    { headers: authHeaders }
  );
  ...
  const createForUnauthRes = http.post(
    `${BASE_URL}/api/contractor`,
    JSON.stringify({
      company_name: 'Unauth Test Contractor',
      inn: '9876543210',
      city: 'СПб',
  ...
  const unauthContractorId = createForUnauthRes.json('id');
  
  // Try to delete without auth
  const deleteUnauthRes = http.del(
    `${BASE_URL}/api/contractor/${unauthContractorId}`,
    null,
    { headers }
  );
  ...
  // Cleanup
  http.del(`${BASE_URL}/api/contractor/${unauthContractorId}`, null, { headers: authHeaders });
  
  console.log('\n✅ DELETE contractor tests completed!');
}

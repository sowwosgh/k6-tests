...
export default function () {
  ...
  const createRes = http.post(
    ...
    JSON.stringify({
      title: 'Test Vacancy for Avatar',
      company_name: 'Test Company',
      city: 'Москва',
      specialization: 'Программист',
      description: 'Test vacancy for avatar upload tests',
      salary_min: 100000,
      salary_max: 150000,
      phone: '+79001234567',
      is_active: true,
  ...
  if (createRes.status !== 200 && createRes.status !== 201) {
    console.error('❌ Failed to create vacancy:', createRes.body);
    return;
  }

  const vacancyId = createRes.json('id');
  console.log(`✅ Vacancy created: ID=${vacancyId}`);
  ...
  const uploadRes = http.post(
    `${BASE_URL}/api/vacancy/${vacancyId}/avatar`,
    { avatar: imageFile },
    { headers: authHeaders }
  );
  ...
  // ===========================================
  // Test 2: Upload Avatar — Non-Existent Vacancy
  // ===========================================
  console.log('\n❌ Test 2: Upload avatar to non-existent vacancy...');
  const uploadWrongRes = http.post(
    `${BASE_URL}/api/vacancy/999999/avatar`,
    { avatar: imageFile },
    { headers: authHeaders }
  );
  ...
  // ===========================================
  // Test 5: Delete Avatar — Non-Existent Vacancy
  // ===========================================
  console.log('\n❌ Test 5: Delete avatar on non-existent vacancy...');
  ...
  // Cleanup
  http.del(`${BASE_URL}/api/vacancy/${vacancyId}`, null, { headers: authJsonHeaders });
  console.log('\n✅ Vacancy avatar tests completed');
}
